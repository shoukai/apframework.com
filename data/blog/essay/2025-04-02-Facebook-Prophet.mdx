---
title: Facebook Prophet 预测商品价格
date: '2025-04-02'
tags: ['Prophet', 'Facebook', '预测时间序列数据', '预测模型', '随手记']
draft: false
summary: '基于加法模型预测时间序列数据 Facebook Prophet 开源项目介绍与应用案例'
---
...
最近咖啡喝完了，想继续补充 UCC 117 咖啡作为口粮，到比价网张上获取价格数据。例如：[https://www.manmanbuy.com/](https://www.manmanbuy.com/) 
...
1. 将时间戳转换为 Prophet 要求的 `ds` 格式
2. 对数据进行异常值处理（使用 Z-score 方法）
3. 对价格进行对数变换，使数据更稳定
4. 计算移动平均和标准差，用于后续分析
...
1. `changepoint_prior_scale`：控制趋势的灵活性，值越小趋势越平滑
2. `seasonality_prior_scale`：控制季节性强度，值越大季节性越明显
3. `seasonality_mode`：季节性模式，可以是加法或乘法
4. `growth`：增长模式，可以是线性或逻辑增长

添加自定义季节性：

- `add_seasonality`：添加自定义周期性，如月度季节性
- `add_country_holidays`：添加特定国家的节假日效应
...
1. 使用 `make_future_dataframe` 创建未来时间点
2. 使用 `predict` 方法生成预测结果
3. 将预测结果从对数空间转换回原始空间
...
![image.png](/static/images/2025-04-02-Facebook-Prophet/image.png)
