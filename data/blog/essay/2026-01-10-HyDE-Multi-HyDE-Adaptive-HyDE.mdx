...
<div className="text-center">
  <img src="/static/images/2026-01-10-HyDE-Multi-HyDE-Adaptive-HyDE/2h-media-spScdgWY-_c-unsplash.jpg" alt="Multi-agent" />
  <p>HyDE（Photo by <a href="https://unsplash.com/@2hmedia?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">2H Media</a> on <a href="https://unsplash.com/photos/a-wooden-table-topped-with-papers-and-a-pen-spScdgWY-_c?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText">Unsplash</a>）</p>
</div>
...
![](/static/images/2026-01-10-HyDE-Multi-HyDE-Adaptive-HyDE/iShot_2026-01-10_11.22.45.png)
...
1.  **初始化**：配置 LangChain 的 LLM 和 Embeddings 客户端。
2.  **构建知识库**：创建一个包含 LangChain 和 RAG 相关知识的临时 Chroma 向量数据库。
3.  **执行检索**：针对查询“如何安装 langchain 库？”，调用 `AsyncHyDE` 进行 Multi-HyDE 检索，并打印耗时和结果。
...
*   **优化效果**：通过 `AsyncHyDE` 的并行生成，我们将耗时控制在了 **20秒左右**（取决于 LLM 响应速度）。相比于串行实现的 60秒+，这是生产环境可接受的延迟。
...
正如代码所示，**并行化**是 Multi-HyDE 落地的前提。
*   **串行瓶颈**：$T_{total} = \sum T_{gen_i} \approx N \times 20s$
*   **并行优化**：$T_{total} = \max(T_{gen_i}) + T_{overhead} \approx 20s$
通过 `asyncio.gather`，我们成功抹平了多视角带来的大部分时间成本。

#### 进阶思路 1：使用更快的模型
HyDE 的核心在于生成**关键词**和**语义向量**，而非生成完美的散文。
*   **行动**：可以将生成假设文档的模型替换为更小、更快的模型（如 `gpt-4o-mini`, `qwen-turbo` 等），将单次生成耗时压缩到 5s 以内，从而实现接近实时（ < 5s）的 Multi-HyDE 检索。
...
- [Enhancing Financial RAG with Agentic AI and Multi-HyDE](https://arxiv.org/abs/2509.16369)
- [Never Come Up Empty: Adaptive HyDE Retrieval for Improving LLM Developer Support](https://arxiv.org/abs/2507.16754v1)
