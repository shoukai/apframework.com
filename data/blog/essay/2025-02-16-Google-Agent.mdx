---
title: Google Agent 白皮书摘要
date: '2025-02-16'
tags: ['AI', 'Agent', 'LLM', '随手记']
draft: false
summary: 'Google Agent 模型、工具、编排层'
---

整理下 Google Agent 文档中的要点内容，部分概念进行扩展，原文链接： [Googel Agent 白皮书](https://drive.google.com/file/d/1oEjiRCTbd54aSdB_eEe3UShxLBWK9xkt/view)
...
![image.png](/static/images/2025-02-16-Google-Agent/image.png)
...
- 函数在*客户端*，而扩展则在*代理端*。
    ...
    ![image.png](/static/images/2025-02-16-Google-Agent/image%201.png)
...
![image.png](/static/images/2025-02-16-Google-Agent/image%202.png)
...
<table>
  ...
  <tr>
    ...
    <td>用例</td>
      <td>
        1. 开发人员希望代理控制与 API 端点<br />
        2. 利用本地内置扩展（即向量搜索，代码编译等)<br />
        3. 多跳规划以及API调用（代理的下一个动作取决于前一个动作的输出）
      </td>
      <td>
        1. 安全验证<br />
        2. 时间限制或者操作顺序（批量操作、人机交互）<br />
        3. 未暴露互联网的 API
      </td>
      <td>
        1. 代理可以直接访问数据<br />
...
3. **扩展、函数和数据存储等工具是代理与外部世界沟通的钥匙**，使它们能够与外部系统交互并访问训练数据以外的知识。扩展在代理和外部 API 之间架起了一座桥梁，使 API 调用得以执行并检索实时信息。函数通过分工为开发人员提供了更细致的控制，使代理能够生成可在客户端执行的函数参数。数据存储为代理提供了对结构化或非结构化数据的访问，从而实现了数据驱动的应用程序。