---
title: '知识提取：LangChain 核心概念与架构精要'
date: '2026-01-17'
...
- **pip install -U** (3⭐): 使用 pip 升级并安装指定 Python 包的命令。
...
#### Core Concepts
- **Agent (智能体)** (5⭐): 能调用工具、理解上下文并执行任务的语言模型应用。
- **System Prompt (系统提示词)** (4⭐): 定义智能体角色和行为的指令，应具体且可操作。
- **Tools (工具)** (5⭐): 允许语言模型调用外部函数以获取数据或执行操作，可依赖运行时上下文和记忆。
- **Structured Output (结构化输出)** (4⭐): 通过定义响应格式（如 JSON Schema），确保模型输出符合预期结构。
- **Memory (记忆)** (5⭐): 使智能体能记住对话历史和上下文，在多次交互中保持状态；生产环境应使用持久化存储。
- **Model Configuration (模型配置)** (3⭐): 根据使用场景设置语言模型参数（如温度、最大 token 数等），不同提供商参数不同。
...
#### Core Concepts
- **Large Language Models (LLMs)** (5⭐): 大语言模型是强大新技术，结合外部数据源后效果更佳，并将推动未来应用向智能体（agentic）方向演进。
- **Agent (智能体)** (5⭐): 未来应用程序将越来越具有智能体特性，即能自主推理并调用工具执行任务，但目前构建可靠生产级智能体仍具挑战。
- **LangChain Philosophy** (4⭐): LangChain 的核心理念是标准化模型接口以避免厂商锁定，并使开发者能轻松使用 LLM 编排复杂的数据与计算流程。
- **ReAct (Reasoning and Acting)** (3⭐): 早期 LangChain 智能体基于 ReAct 范式，通过 LLM 生成 JSON 来表示工具调用，实现推理与行动的结合。
- **Function Calling** (4⭐): OpenAI 引入的函数调用功能允许模型直接生成结构化工具调用载荷，取代了早期依赖 JSON 解析的方式，成为 LangChain 推荐的标准方法。
- **LangGraph** (5⭐): LangGraph 是 LangChain 推出的开源低层编排库，提供流式处理、持久化执行、短期记忆和人机协同等能力，成为构建可靠 AI 应用的首选方式。
- **LangChain 1.0** (4⭐): LangChain 1.0 统一所有链（Chains）和智能体为单一基于 LangGraph 的高级 Agent 抽象，并引入标准化的多模态消息内容格式。
- **langchain-classic** (3⭐): 为兼容旧版用户，LangChain 1.0 将原有 Chains 和 Agents 移至独立的 langchain-classic 包，但官方推荐迁移到新架构。
- **Multimodal Model APIs** (4⭐): 现代模型 API 支持图像、视频等多模态输入，LangChain 在 langchain-core 中更新消息格式以标准化此类输入。
- **LangSmith** (4⭐): LangSmith 是 LangChain Inc. 提供的可观测性与评估平台，旨在解决智能体可靠性难题，并与 LangChain 深度集成。
...
本文档介绍了 LangChain 的 React Hook `useStream`，它用于与 LangGraph 的流式功能无缝集成，支持消息流处理、自动状态管理、对话分支、自定义事件、多智能体流、人工干预（human-in-the-loop）等高级特性，并提供类型安全和 UI 无关的设计。

#### Core Concepts
- **useStream (React Hook)** (5⭐): LangChain 提供的 React Hook，用于连接 LangGraph 流式接口，自动处理消息流、状态、错误、分支等逻辑。
- **Messages streaming** (4⭐): 将流式返回的消息块逐步拼接成完整消息，实现流畅的 AI 对话体验。
- **Automatic state management** (4⭐): 自动管理消息、中断、加载状态和错误，无需手动维护复杂状态。
- **Conversation branching** (4⭐): 从任意历史消息点创建对话分支，支持编辑、重生成等高级交互。
- **Thread management** (4⭐): 通过 threadId 管理会话线程，支持页面刷新后自动恢复对话。
- **Optimistic updates** (3⭐): 在请求发出前先更新 UI 状态，提升用户响应速度和体验。
- **Human-in-the-loop (人工干预)** (4⭐): 当 Agent 需要人类批准工具调用时，useStream 可捕获中断并提示用户操作。
- **Multi-agent streaming** (4⭐): 在多智能体系统中，通过消息元数据区分不同节点生成的内容，支持并行 LLM 输出。
- **Custom streaming events** (4⭐): 通过 writer 发送自定义事件（如进度条、文件操作），并在前端通过 onCustomEvent 处理。
- **Type-safe streaming** (4⭐): 配合 createAgent 或 StateGraph 使用时，自动推导工具调用和状态类型，提供完整 TypeScript 支持。
- **Reasoning models** (3⭐): 针对支持推理过程的模型（如 OpenAI Reasoning Models），需从消息内容中提取并单独展示思考过程。
- **Custom transport** (3⭐): 通过 FetchStreamTransport 连接非标准或自定义的流式 API 端点。
